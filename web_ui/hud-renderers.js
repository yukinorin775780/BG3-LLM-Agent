...
(() => {
  ...
  function updateCompanionBarkQaState() {
    ...
    window.__BG3_QA_STATE__ = {
      ...((window.__BG3_QA_STATE__ && typeof window.__BG3_QA_STATE__ === "object") ? window.__BG3_QA_STATE__ : {}),
  ...
  function normalizeSpeaker(id) {
    ...
    if (/laezel|莱埃泽尔/.test(raw)) return "laezel";
    if (/gribbo|格里布|格里波/.test(raw)) return "gribbo";
  ...
  function stanceBarkText(actor, stance) {
    ...
    if (key === "execute") return actor === "laezel" ? "End this. Mercy feeds weakness." : "Kill him before he talks us to death.";
