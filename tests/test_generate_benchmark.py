...
def test_estimate_text_tokens_is_stable_for_chinese_english_and_json():
    ...
    assert bench.estimate_text_tokens("地精巡逻兵") == 5
    assert bench.estimate_payload_tokens({"npc": "goblin_1", "input": "move north"}) == 12


def test_naive_full_state_prompt_exceeds_scoped_prompt():
    step = EvalStep(
        id="actor",
        user_input="阿斯代伦，检查一下这具尸体。",
        intent="chat",
        payload={"target": "astarion"},
    )
    response = {"responses": [{"speaker": "astarion", "text": "我看看。"}], "journal_events": []}
    snapshot = {
        "game_state": {
