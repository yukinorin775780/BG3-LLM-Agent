...
from core.campaigns.necromancer_lab import detect_astarion_memory_echo_context
...
def _with_rebuke_history(state: dict) -> dict:
    ...
    out["entities"] = {
        **dict(state.get("entities") or {}),
        "astarion": {**dict(state["entities"]["astarion"]), "affection": 7},
    }
    return out
...
def test_memory_echo_helper_noops_without_prior_history():
    ...
    assert detect_astarion_memory_echo_context(state, "阿斯代伦，你怎么看？", {}) is None
...
def test_rebuke_history_surfaces_resentful_response_journal_and_flag_without_affection_delta():
    ...
    result = _run_chat_turn(state, "阿斯代伦，你怎么看这件事？")

    response_text = "\n".join(text for _, text in result.get("speaker_responses", []))
    assert "现在又需要我" in response_text
    assert "闭嘴" in response_text
    assert any(
        "[记忆回响] astarion -> rebuked_by_player" in line
        for line in result.get("journal_events", [])
    )
    assert result["flags"]["necromancer_lab_astarion_memory_echo_seen"] is True
    assert result["flags"]["necromancer_lab_astarion_rebuke_echo_seen"] is True
    assert result["entities"]["astarion"]["affection"] == 7


def test_side_history_surfaces_complicit_response_and_journal():
    state = _with_side_history(_build_lab_state())
    result = _run_chat_turn(state, "Astarion, what do you think?")

    response_text = "\n".join(text for _, text in result.get("speaker_responses", []))
    assert "一起" in response_text
    assert "默契" in response_text
    assert any(
        "[记忆回响] astarion -> sided_with_player" in line
        for line in result.get("journal_events", [])
    )
    assert result["flags"]["necromancer_lab_astarion_complicity_echo_seen"] is True
    assert result["entities"]["astarion"]["affection"] == 7


def test_rebuke_echo_does_not_block_key_guidance():
    state = _with_rebuke_history(_build_lab_state())
    result = _run_chat_turn(state, "阿斯代伦，怎么打开实验室门？")
...
def test_rebuke_echo_does_not_block_astarion_trap_disarm():
    state = _with_rebuke_history(_build_lab_state())
    state["flags"] = {
        **dict(state.get("flags") or {}),
...
def test_astarion_private_memory_does_not_leak_to_other_actor_views():
    state = _with_rebuke_history(_build_lab_state())

    shadowheart_view = build_actor_view(state, "shadowheart")
    laezel_view = build_actor_view(state, "laezel")

    assert "记住这笔账" not in repr(shadowheart_view)
    assert "记住这笔账" not in repr(laezel_view)
    assert not hasattr(shadowheart_view.other_entities["astarion"], "memory_notes")
    assert not hasattr(laezel_view.other_entities["astarion"], "memory_notes")
