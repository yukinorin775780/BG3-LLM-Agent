...
def _build_secret_study_base_state(user_input: str = "") -> dict:
    ...
    state["environment_objects"] = {
        ...
        "chemical_notes": {
            ...
            "type": "readable",
            "name": "化学残页",
            "lore_id": "necromancer_chemical_notes",
            "x": 15,
            "y": 4,
        },
        "iron_key_sketch": {
            ...
            "type": "readable",
            "name": "重铁钥匙草图",
            "lore_id": "necromancer_iron_key_sketch",
            "x": 16,
            "y": 3,
        },
    }
...
def test_act3_secret_study_companion_observations_use_actor_runtime():
    ...
    state["current_speaker"] = "astarion"
    state["speaker_queue"] = ["shadowheart", "laezel"]
    state["intent"] = "CHAT"
    state["intent_context"] = {
        "action_actor": "player",
    ...
    assert "[书房观察] astarion -> practical_clues" in journal
    assert "[书房观察] shadowheart -> necromancy_pollution" in journal
    assert "[书房观察] laezel -> tactical_impatience" in journal
    assert drained.get("player_inventory", {}) == state.get("player_inventory", {})
    assert drained.get("combat_active") is not True


def test_act3_diary_success_information_advantage_then_gribbo_pressure(monkeypatch):
    monkeypatch.setattr("core.graph.nodes.lore.settings.API_KEY", None)
    state = _build_secret_study_base_state("用奥术知识阅读 necromancer_diary。")
    state["flags"]["act3_secret_study_entered"] = True
    state["intent"] = "READ"
    state["intent_context"] = {
        "action_actor": "player",
    ...
    assert result["flags"]["act3_gribbo_potion_truth_known"] is True
    assert result["flags"]["act3_heavy_key_hint_known"] is True
    assert result["flags"]["act3_party_knows_gribbo_truth"] is True
    assert any("Gribbo" in note and "heavy_iron_key" in note for note in result["actor_runtime_state"]["player"]["memory_notes"])
    assert any(
        "Gribbo" in note and "毒气" in note and "heavy_iron_key" in note
    ...
    pressure_state = {
        **result,
        ...
        "active_dialogue_target": "gribbo",
        "target": "",
        "user_input": "日记里写了你喝下死灵狂暴灵药，钥匙和毒气真相都和你有关。",
        "current_speaker": "",
        "speaker_queue": [],
    }
...
def test_act3_diary_failure_fragment_only_blocks_gribbo_pressure(monkeypatch):
    monkeypatch.setattr("core.graph.nodes.lore.settings.API_KEY", None)
    state = _build_secret_study_base_state("阅读 necromancer_diary。")
    state["flags"]["act3_secret_study_entered"] = True
    ...
    assert result["flags"]["act3_gribbo_potion_truth_known"] is False
    assert "act3_party_knows_gribbo_truth" not in result["flags"]
    assert "__party_shared__" not in result.get("actor_runtime_state", {})
    assert any("碎片" in note or "地精、箱子、毒气" in note for note in result["actor_runtime_state"]["player"]["memory_notes"])
    ...
    pressure_state = {
        **result,
        ...
        "active_dialogue_target": "gribbo",
        "target": "",
        "user_input": "日记里写了你喝下死灵狂暴灵药，把钥匙给我。",
        "current_speaker": "",
        "speaker_queue": [],
    }
    with patch(
...
def test_act3_diary_succeeds_after_context_gathered(monkeypatch):
    ...
    state["entities"]["player"]["ability_scores"] = {"INT": 8}
    state["intent"] = "READ"
    state["intent_context"] = {"action_actor": "player", "action_target": "necromancer_diary"}
...
def test_act3_diary_can_still_fail_without_context(monkeypatch):
    ...
    assert lore_patch["flags"]["act3_gribbo_potion_truth_known"] is False
...
def test_full_path_uses_study_context_before_diary_truth():
    golden_path = Path("evals/golden/necromancer_lab_full_path_act2_to_act4_truth_negotiation.yaml")
...
def test_act3_chemical_notes_and_key_sketch_flags(monkeypatch):
    ...
    assert any("shadowheart -> necromancy_pollution" in line for line in chemical["journal_events"])
    ...
    assert any("astarion -> practical_clues" in line for line in sketch["journal_events"])
